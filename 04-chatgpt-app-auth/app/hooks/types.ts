...
export interface UploadFileArgs {
  /**
   * Function that returns an ArrayBuffer with the file data.
   * Same as File/Blob: () => file.arrayBuffer()
   */
  arrayBuffer: () => Promise<ArrayBuffer> | ArrayBuffer;

  /** Optional, but convenient for UI */
  name?: string;
  mimeType?: string;
}
...
export interface ToolExtra {
  /** AbortSignal so you can cancel tool execution */
  signal: AbortSignal;

  /** May be undefined — better not to rely on it */
  sessionId?: string;

  /** Metadata from OpenAI (subject, locale, userLocation, userAgent, ...) */
  _meta?: OpenAIMeta;

  /** Authorization (if configured); currently undefined for you */
  authInfo?: unknown;

  /** Request ID, useful for logging */
  requestId?: number | string;

  /** HTTP info about the request (if available) */
...
export interface AuthInfo {
  ...
  clientId?: string;
  scopes?: string[];
  expiresAt?: string; // ISO date/time, if present
